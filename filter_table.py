...
def filter_table(sampleId, referral, gaps_path, bedfile_path):
    ...
    if (referral=="Melanoma" or referral=="Lung" or referral=="GIST" or referral=="Colorectal"):
        ...
        if (file.shape[0]!=0):
            ...
            grouped_file=file.filter(items=['Chr', 'Start', 'End','Info', 'Gene', 'Counts', 'Percentage' ])
            grouped_file.to_csv(gaps_path+sampleId+"_"+referral+"_output.csv", sep=',', index=False)

    else:

        grouped_file=pandas.read_csv(gaps_path+sampleId+"_"+referral+"_hotspots.gaps", sep='\t', names=['Chr','Start', 'End', 'Info'])
...
if __name__ == '__main__':
    ...
    filter_table(sampleId, referral, gaps_path, bedfile_path)
